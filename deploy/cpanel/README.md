...
- **Untuk database yang sudah jalan (Update ke versi terbaru)**: buka file [`update-023-to-039.sql`](update-023-to-039.sql), salin isinya, tempel, **Execute**. Script ini meng-update semua pages, news & articles, services & products ke format bilingual (ID & EN 100% lengkap), menambahkan produk Rittal terbaru, memperbaiki spesifikasi agar tidak duplikat, dan menyinkronkan status kelengkapan dwi-bahasa di Admin CMS (termasuk halaman About, News, dan Services yang sebelumnya berstatus ID Kosong atau EN Kosong serta Page Builder).
