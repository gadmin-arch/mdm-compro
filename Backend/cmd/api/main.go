...
func main() {
	...
	// Refuse to boot production with a guessable signing key: every session
	// token could be forged with it.
	if strings.EqualFold(cfg.AppEnv, "production") {
		if cfg.JWTSecret == "change-me-in-production" || len(cfg.JWTSecret) < 32 {
			logger.Error("JWT_SECRET must be a random string of at least 32 characters in production")
			os.Exit(1)
		}
	}
