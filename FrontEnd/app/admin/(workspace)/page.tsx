...
const moduleCards: ModuleCard[] = [
  { label: "Pages", key: "pages", icon: FileText, href: "/admin/pages", content: true, roles: ["owner", "admin"] },
  { label: "Services", key: "services", icon: Wrench, href: "/admin/services", content: true, roles: ["owner", "admin", "user"] },
  { label: "Products", key: "products", icon: Package, href: "/admin/products", content: true, roles: ["owner", "admin", "user"] },
  { label: "News", key: "news", icon: Newspaper, href: "/admin/news", content: true, roles: ["owner", "admin", "user"] },
  { label: "Careers", key: "careers", icon: Briefcase, href: "/admin/careers", content: true, roles: ["owner", "admin", "user"] },
  { label: "Users", key: "users", icon: Users, href: "/admin/users", roles: ["owner", "admin"] },
  { label: "Archive", key: "archive", icon: Archive, href: "/admin/archive", roles: ["owner", "admin"] },
]

const statusOrder = ["published", "draft", "scheduled", "archived"] as const

export default async function AdminPage() {
  const [dashboard, contacts, activity, user] = await Promise.all([
    ...
    adminFetch<{ id: string; role: "owner" | "admin" | "user" }>("/profile").catch(swallowApiError),
  ])
  ...
  const visibleModules = moduleCards.filter((m) => !m.roles || m.roles.includes(role))
  ...
  return (
    <>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-2">
        ...
        {role !== "user" && (
          <Button asChild variant="outline">
            <Link href="/admin/navigation" prefetch={true}>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        {role !== "user" && (
          <section className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-[#0b0f17]">
            <div className="flex items-center gap-2">
              <Inbox className="h-4 w-4 text-muted-foreground" />
              <h2 className="font-display text-lg font-semibold text-foreground">Contact Inquiries</h2>
              <Button asChild size="sm" variant="outline" className="ml-auto">
                <Link href="/admin/contacts" prefetch={true}>
                  Manage
            ...
            <div className="mt-4 space-y-3">
              {inquiries.map((inquiry) => (
                <article key={inquiry.id} className="rounded-xl border border-border bg-secondary/20 p-3">
